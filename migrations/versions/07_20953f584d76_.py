# coding=utf-8
"""在作品集中增加风格，删除photo_work_image，直接使用photo_work关联image
...
def upgrade():
    ...
    op.add_column('photo_collection', sa.Column('style_id', sa.Integer(),
                                                nullable=True))
    op.create_foreign_key('photo_collection_style_id_fkey', 'photo_collection',
                          'enum_values', ['style_id'], ['id'])
    op.drop_table('photo_work_image')
    op.add_column('photo_work', sa.Column('image_id', sa.Integer(),
                                          nullable=True))
    op.create_foreign_key('photo_work_image_id_fkey', 'photo_work', 'image',
                          ['image_id'], ['id'])
...
def downgrade():
    # commands auto generated by Alembic - please adjust! ###
    op.drop_constraint('photo_work_image_id_fkey', 'photo_work',
                       type_='foreignkey')
    ...
    op.create_table('photo_work_image', sa.Column('id', sa.INTEGER(),
                    ...
                    sa.ForeignKeyConstraint(['image_id'],
                                            ['image.id'],
                                            name='photo_work_image_image_id_fkey'),
                    sa.ForeignKeyConstraint(['photo_work_id'],
                                            ['photo_work.id'],
                                            name='photo_work_image_photo_work_id_fkey'),
                    sa.PrimaryKeyConstraint('id', name='photo_work_image_pkey')
                    )
    op.drop_constraint('photo_collection_style_id_fkey', 'photo_collection',
                       type_='foreignkey')
